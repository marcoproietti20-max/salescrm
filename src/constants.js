...
export function parseDate(str) {
  ...
  const p = str.match(/^(\d{1,2})[\/\-](\d{1,2})$/);
  if (p) {
    const now = new Date(); const day = p[1].padStart(2,'0'); const mon = p[2].padStart(2,'0');
    const yr = now.getFullYear(); const cand = `${yr}-${mon}-${day}`;
    return new Date(cand) < now ? `${yr+1}-${mon}-${day}` : cand;
  }
  return str;
}
...
export function getFatturato(c) {
  ...
  return list.reduce((s, ct) => {
    if (ct.prodotti?.length) return s + ct.prodotti.reduce((ps, p) => ps + (Number(p.importo) || 0), 0);
    return s + (Number(ct.totale) || 0);
  }, 0);
}
...
export function parseCSVRow(row, stages) {
  ...
  const importoContratto = Number((row['Importo Contratto'] || row['importo contratto'] || '0').toString().replace(/[€,\.]/g, '').replace(',', '.')) || 0;
  ...
  if (importoContratto > 0) {
    const prodotti = prodottiRaw ? prodottiRaw.split(',').map(p => {
      const parts = p.trim().split(':');
      return { id: uid(), categoria: parts[0]?.trim() || '', nome: '', importo: Number(parts[1]?.trim()) || 0, durataM };
    }) : [];
  ...
  return {
    ...
    history: (() => {
      const hist = [];
      const fu = parseDate(row['Follow Up'] || row['follow up'] || '');
      const note = (row['Note'] || row['note'] || '').trim();
      if (fu || note) hist.push({ id: uid(), type: 'note', date: new Date().toISOString().slice(0, 10), text: note || 'Importato da CSV', followup: fu });
      return hist;
    })(),
    customData: {},
  };
