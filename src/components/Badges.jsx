...
export function StageBadge({ name, stages }) {
  const s=stages?.find(x=>x.name===name); const color=s?s.color:'#888';
...
export function FonteBadge({ name }) {
  ...
  const f=FONTI.find(x=>x.name===name); const color=f?f.color:'#888';
...
export function EsitoBadge({ name }) {
  if (!name) return <span className="text-muted fs-12">—</span>;
  const e=ESITI.find(x=>x.name===name); const color=e?e.color:'#888';
...
export function PropostaBadge({ name }) {
  if (!name) return <span className="text-muted fs-12">—</span>;
  const p=PROPOSTE.find(x=>x.name===name); const color=p?p.color:'#888';
  return <span className="badge" style={{background:color+'18',color,border:`1px solid ${color}44`}}>{name}</span>;
}
export function StatoBadge({ name }) {
  if (!name) return null;
  const s=STATI_APPT.find(x=>x.name===name); const color=s?s.color:'#888';
