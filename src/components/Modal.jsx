...
export default function Modal({ modal, setModal, contacts, stages, customFields,
  ...
  if (modal.type === 'appt') return (
    <ApptForm
      ...
      prefDate={modal.data.prefDate}
      stages={stages}
      contacts={contacts}
      onSave={(cid, appt, fase, newContactData) => {
        if (newContactData) {
          saveContact({
            ...newContactData,
          ...
          showToast('Contatto e appuntamento salvati');
        } else if (cid) {
          updateContact(cid, c => {
            const h = [...(c.history || [])];
  ...
  if (modal.type === 'contratto') return (
    <ContrattoForm contact={modal.data.contact} contratto={modal.data.contratto} isEdit={modal.data.isEdit}
      ...
      onDelete={(cid, ctid) => {
        updateContact(cid, c => ({ ...c, contratti: (c.contratti || []).filter(x => x.id !== ctid) }));
        showToast('Contratto eliminato'); close();
      }}
      onClose={close} />
  );
...
function Overlay({ onClose, children, wide }) {
  return (
    <div className="modal-overlay" onClick={e => e.target === e.currentTarget && onClose()}>
      <div className="modal-box" style={wide ? { width: 700 } : {}}>{children}</div>
    </div>
  );
}
    ...
    <Overlay onClose={onClose}>
      ...
      <div className="modal-body">
        ...
        <div className="form-group">
          ...
          <textarea className="form-control" style={{ minHeight: 70 }} value={f.noteInterne} onChange={e => s('noteInterne', e.target.value)} placeholder="Note private sul contatto." />
        </div>
        ...
        <div style={{ borderTop: '1px solid var(--border)', paddingTop: 14, marginTop: 4 }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 }}>
            ...
            <button type="button" className="btn btn-sm btn-primary"
              onClick={() => s('contratti', [...(f.contratti || []), { id: uid(), tipo: 'Nuovo', nuovoFatturato: 0, prodotti: [], dataInizio: '', totale: 0 }])}>
              + Aggiungi contratto
            </button>
          </div>
          {(f.contratti || []).length === 0 && <div className="fs-12 text-muted" style={{ marginBottom: 8 }}>Nessun contratto</div>}
            ...
            <div key={ct.id || ci} style={{ background: 'var(--green-lt)', border: '1px solid rgba(27,122,62,0.3)', borderRadius: 'var(--r)', padding: '12px 14px', marginBottom: 8 }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 8 }}>
                ...
                <button type="button" className="btn btn-sm btn-danger"
                  onClick={() => s('contratti', (f.contratti || []).filter((_, i) => i !== ci))}>× Elimina</button>
              </div>
              <div className="form-row" style={{ marginBottom: 8 }}>
                <div className="form-group" style={{ margin: 0 }}>
                  <label className="form-label">Tipo</label>
                  <select className="form-control" value={ct.tipo || 'Nuovo'}
                    onChange={e => s('contratti', (f.contratti || []).map((x, i) => i === ci ? { ...x, tipo: e.target.value } : x))}>
                ...
                <div className="form-group" style={{ margin: 0 }}>
                  ...
                  <input className="form-control" type="date" value={ct.dataInizio || ''}
              ...
              {(ct.prodotti || []).map((p, pi) => (
                <div key={p.id || pi} style={{ display: 'flex', gap: 6, marginBottom: 6, flexWrap: 'wrap', alignItems: 'center' }}>
                  <select className="form-control" style={{ flex: 2, minWidth: 120 }} value={p.categoria || ''}
                    onChange={e => s('contratti', (f.contratti || []).map((x, i) => i === ci ? { ...x, prodotti: x.prodotti.map((pp, j) => j === pi ? { ...pp, categoria: e.target.value } : pp) } : x))}>
...
function ApptForm({ contactId, appt, stages, contacts, onSave, onDelete, onClose, prefDate }) {
  ...
  const [selectedCid, setSelectedCid] = useState(contactId || null);
  const [createNew, setCreateNew] = useState(false);
  const [newContact, setNewContact] = useState({ nome: '', azienda: '', telefono: '', email: '' });
  const s = (k, v) => setF(p => ({ ...p, [k]: v }));

  const selectedContact = selectedCid ? contacts?.find(c => c.id === selectedCid) : null;
  ...
  const handleSave = () => {
    if (createNew) {
      const nome = newContact.nome || searchQ;
      if (!nome.trim()) return alert('Inserisci il nome del contatto');
      onSave(null, { ...f, type: 'appt' }, fase, { ...newContact, nome });
    } else {
      const cid = selectedCid || contactId;
      if (!cid) return alert('Seleziona un contatto esistente o creane uno nuovo');
      onSave(cid, { ...f, type: 'appt' }, fase, null);
    }
  };
  ...
  return (
    <Overlay onClose={onClose}>
      <div className="modal-header">
        <span className="modal-title">{a.id ? 'Aggiorna appuntamento' : 'Nuovo appuntamento'}</span>
        <button className="modal-close" onClick={onClose}>×</button>
      </div>
      <div className="modal-body">
        {!a.id && (
          <div className="form-group">
            ...
            {selectedContact && !createNew ? (
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, background: 'var(--accent-lt)', border: '1px solid var(--accent-mid)', borderRadius: 'var(--r)', padding: '8px 12px' }}>
                ...
                <button className="btn btn-sm" onClick={() => { setSelectedCid(null); setSearchQ(''); setCreateNew(false); }}>Cambia</button>
              </div>
            ) : createNew ? (
              <div style={{ background: 'var(--accent-lt)', border: '1px solid var(--accent-mid)', borderRadius: 'var(--r)', padding: 12 }}>
                <div className="form-label" style={{ marginBottom: 8 }}>Nuovo contatto</div>
                <div className="form-row" style={{ marginBottom: 8 }}>
                  <input className="form-control" placeholder="Nome *" value={newContact.nome || searchQ}
                    onChange={e => setNewContact(p => ({ ...p, nome: e.target.value }))} autoFocus />
                  <input className="form-control" placeholder="Azienda"
                    value={newContact.azienda} onChange={e => setNewContact(p => ({ ...p, azienda: e.target.value }))} />
                </div>
                <div className="form-row">
                  <input className="form-control" placeholder="Telefono"
                    value={newContact.telefono} onChange={e => setNewContact(p => ({ ...p, telefono: e.target.value }))} />
                  <input className="form-control" placeholder="Email"
                    value={newContact.email} onChange={e => setNewContact(p => ({ ...p, email: e.target.value }))} />
                </div>
                <button className="btn btn-sm" style={{ marginTop: 8 }} onClick={() => setCreateNew(false)}>← Cerca esistente</button>
              </div>
            ) : (
              <div>
                <input className="form-control" placeholder="Cerca contatto per nome o azienda..."
                  value={searchQ} onChange={e => setSearchQ(e.target.value)} autoFocus />
                {searchResults.length > 0 && (
                  <div style={{ border: '1px solid var(--border2)', borderRadius: 'var(--r)', marginTop: 4, overflow: 'hidden', maxHeight: 200, overflowY: 'auto' }}>
                    {searchResults.map(c => (
                      <div key={c.id}
                        onClick={() => { setSelectedCid(c.id); setSearchQ(''); }}
        ...
        <div className="form-row">
          <div className="form-group">
            ...
            <input className="form-control" type="datetime-local" value={f.date} onChange={e => s('date', e.target.value)} />
          </div>
          <div className="form-group">
        ...
        <div className="form-group">
          ...
          <textarea className="form-control" value={f.esito} onChange={e => s('esito', e.target.value)} placeholder="Come è andato?" />
        </div>
        <div className="form-group">
  ...
  return (
    <Overlay onClose={onClose}>
      ...
      <div className="modal-body">
        ...
        {q && results.map(c => (
          <div key={c.id} onClick={() => setSel(c)} style={{ padding: '10px 12px', borderRadius: 'var(--r)', border: `2px solid ${sel?.id === c.id ? 'var(--accent)' : 'var(--border)'}`, marginBottom: 6, cursor: 'pointer', background: sel?.id === c.id ? 'var(--accent-lt)' : 'var(--bg2)' }}>
        ...
        {sel && <div className="info-box amber" style={{ marginTop: 12 }}>⚠️ <strong>{sel.nome}</strong> verrà eliminato e unito a <strong>{contact.nome}</strong>. Irreversibile.</div>}
      </div>
      <div className="modal-footer">
        <button className="btn" onClick={onClose}>Annulla</button>
        <button className="btn btn-primary" disabled={!sel} onClick={() => { if (sel && window.confirm('Confermi?')) onMerge(contact.id, sel.id); }}>Unisci</button>
      </div>
    </Overlay>
  );
}
      ...
      <div className="modal-box" style={{ width: 700 }}>
        ...
        <div className="modal-body" style={{ maxHeight: '75vh', overflowY: 'auto' }}>
          <div className="info-grid" style={{ marginBottom: 16 }}>
            ...
            <div className="info-item"><label>Importo proposta</label><span style={{ fontWeight: 600, color: 'var(--accent-dk)' }}>{getPreventivato(c) > 0 ? '€' + getPreventivato(c).toLocaleString('it-IT') : '—'}</span></div>
          ...
          {getContratti(c).length > 0 && (
            <div style={{ marginBottom: 16 }}>
              ...
              {getContratti(c).map((ct, i) => (
                <div key={ct.id || i} style={{ background: 'var(--green-lt)', border: '1px solid rgba(27,122,62,0.3)', borderRadius: 'var(--r)', padding: '10px 14px', marginBottom: 8 }}>
                  ...
                  {(ct.prodotti || []).length > 0 && (
                    <div className="fs-12">
                      {ct.prodotti.map(p => (
                        <span key={p.id} style={{ display: 'inline-block', background: 'white', border: '1px solid rgba(27,122,62,0.3)', borderRadius: 4, padding: '2px 8px', marginRight: 6, marginTop: 4 }}>
          ...
          {c.noteInterne && (
            <div style={{ marginBottom: 16, background: 'var(--bg3)', borderRadius: 'var(--r)', padding: '12px 14px' }}>
              <div className="form-label" style={{ marginBottom: 6 }}>Note interne</div>
              <div className="fs-12 text-muted" style={{ lineHeight: 1.6 }}>{c.noteInterne}</div>
            </div>
          )}
          ...
          <div style={{ borderTop: '1px solid var(--border)', paddingTop: 14 }}>
            ...
            {hist.map(h => {
              if (h.type === 'appt') {
                const bc = { 'Svolto': 'var(--green)', 'Da rifissare': 'var(--amber)', 'Non effettuato': 'var(--red)', 'Non si è presentato': 'var(--red)', 'Programmato': 'var(--accent)' }[h.stato] || 'var(--accent)';
                return (
                  <div key={h.id} className="history-item appt" style={{ borderLeftColor: bc }}>
                    ...
                    <button className="btn btn-sm" style={{ marginTop: 6 }}
                      onClick={() => { onClose(); setTimeout(() => setModal({ type: 'appt', data: { contactId: c.id, appt: h } }), 100); }}>Aggiorna</button>
                  </div>
                );
              }
              const fs = h.followup ? (h.followup < today ? 'scaduto' : h.followup === today ? 'oggi' : 'futuro') : '';
              const fc = { scaduto: 'var(--red)', oggi: 'var(--amber)', futuro: 'var(--accent)' }[fs] || '';
              return (
                <div key={h.id} className="history-item note">
                  ...
                  <div style={{ display: 'flex', gap: 6, marginTop: 6 }}>
                    ...
                    <button className="btn btn-sm btn-danger"
                      onClick={() => updateContact(c.id, ct => ({ ...ct, history: (ct.history || []).map(x => x.id === h.id ? { ...x, followup: '' } : x) }))}>× Elimina follow-up</button>
                  </div>
                </div>
              );
            })}
