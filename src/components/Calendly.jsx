...
import { uid, parseDate, parseCSVRow } from '../constants';
...
export default function Calendly({ contacts, stages, setContacts, gsCfg, brand, syncFromGoogleSheet, importFromCSV, showToast }) {
  ...
  const importManual = () => {
    ...
    setContacts(p => [...p, { id: uid(), nome: form.nome, azienda: form.azienda, email: form.email, telefono: form.telefono, fase: stages[1]?.name || stages[0]?.name, fonte: 'Calendly', categoria: '', esito: '', proposta: '', importoProposta: 0, dataChiusura: '', contratti: [], testoProposta: '', history, customData: {} }]);
  ...
  return (
    <>
      <div className="topbar"><span className="page-title">Calendly & Importazione</span></div>
      <div className="content">
        ...
        <div className="card">
          ...
          <p className="text-muted fs-12" style={{ marginBottom: 14, lineHeight: 1.6 }}>
            Zapier popola il Google Sheet ad ogni nuova prenotazione Calendly. Clicca <strong>Sincronizza ora</strong> per importare i nuovi contatti — i duplicati vengono ignorati e i contratti esistenti non vengono sovrascritti.
          </p>
        ...
        <div className="card">
          ...
          <p className="text-muted fs-12" style={{ marginBottom: 14, lineHeight: 1.6 }}>
            Carica un file CSV o Excel con i contatti da importare. Usa il template come riferimento per le colonne.
          </p>
          <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap', marginBottom: 14 }}>
            <button className="btn" onClick={() => fileRef.current?.click()} disabled={importing}>
              {importing ? '⏳ Importazione...' : '📁 Carica file CSV / Excel'}
            </button>
          ...
          <div className="info-box blue" style={{ marginTop: 14 }}>
            <div><strong>Colonne supportate:</strong> Nome, Azienda, Email, Telefono, Categoria, Fonte, Fase, Esito, Proposta, Importo Proposta, Data Chiusura, Follow Up, Note, Importo Contratto, Prodotti, Data Inizio Contratto, Durata Mesi</div>
          </div>
        </div>

        {/* Manual import */}
        <div className="card">
          <div className="card-title">Importa prenotazione manualmente</div>
